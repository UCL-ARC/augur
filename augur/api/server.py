...
def flaskify(function: Any) -> Any:
    ...
    def generated_function(*args, **kwargs):
        ...
        return Response(
            response=route_transform(function, args, kwargs, **request.args.to_dict()),
            status=200,
...
def add_standard_metric(function: Any, endpoint: str) -> None:
    ...
    repo_group_endpoint = (
        f"/{app.augur_api_version}/repo-groups/<repo_group_id>/{endpoint}"
    )
...
def get_connection(
    table, cursor_field_name, connection_class, after, limit, extra_condition=False
):
    ...
    return connection_class(
        items=items,
        page_info=PageInfoType(next_cursor=next_cursor, has_next_page=has_next_page),
    )
...
class RepoType(SQLAlchemyObjectType):
    ...
    issues = graphene.Field(
        lambda: IssueConnection,
        after=graphene.String(),
        limit=graphene.Int(default_value=10),
    )
    prs = graphene.Field(
        lambda: PullRequestConnection,
        after=graphene.String(),
        limit=graphene.Int(default_value=10),
    )
    messages = graphene.Field(
        lambda: MessageConnection,
        after=graphene.String(),
        limit=graphene.Int(default_value=10),
    )
    ...
    def resolve_issues(self, info, after=None, limit=None):
        ...
        issue_connection = get_connection(
            Issue, "issue_id", IssueConnection, after, limit, condition
        )
        return issue_connection

    def resolve_prs(self, info, after=None, limit=None):
        condition = {"field_name": "repo_id", "value": self.repo_id}
        pr_connection = get_connection(
            PullRequest,
            "pull_request_id",
    ...
    def resolve_messages(self, info, after=None, limit=None):
        condition = {"field_name": "repo_id", "value": self.repo_id}
        messages_connection = get_connection(
            Message, "msg_id", MessageConnection, after, limit, condition
        )
...
class Query(graphene.ObjectType):
    repos = graphene.Field(
        RepoConnection, after=graphene.String(), limit=graphene.Int(default_value=10)
    )
    repo = graphene.Field(RepoType, id=graphene.Int())

    issues = graphene.Field(
        IssueConnection, after=graphene.String(), limit=graphene.Int(default_value=10)
    )
    ...
    messages = graphene.Field(
        MessageConnection, after=graphene.String(), limit=graphene.Int(default_value=10)
    )
    commits = graphene.Field(
        CommitConnection, after=graphene.String(), limit=graphene.Int(default_value=10)
    )

    contributors = graphene.Field(
    ...
    def resolve_issues(self, info, after=None, limit=None):
        issue_connection = get_connection(
            Issue, "issue_id", IssueConnection, after, limit
        )
        return issue_connection
    ...
    def resolve_prs(self, info, after=None, limit=None):
        pr_connection = get_connection(
            PullRequest, "pull_request_id", PullRequestConnection, after, limit
        )
        return pr_connection
    ...
    def resolve_pr(self, info, id):
        return (
            ...
            .filter(PullRequest.pull_request_id == id)
            .first()
        )

    def resolve_messages(self, info, after=None, limit=None):
        messages_connection = get_connection(
            Message, "msg_id", MessageConnection, after, limit
        )
        return messages_connection

    def resolve_commits(self, info, after=None, limit=None):
        commit_connection = get_connection(
            Commit, "cmt_id", CommitConnection, after, limit
        )
        return commit_connection

    def resolve_contributors(self, info, after=None, limit=None):
        contributors_connection = get_connection(
            Contributor, "cntrb_id", ContributorConnection, after, limit
        )
...
def status():
    ...
    Health check route
    """
    status = {"status": "OK", "version": augur_code_version}
    return Response(
...
app.add_url_rule(
    f"/{app.augur_api_version}/graphql",
    view_func=AuthenticatedGraphQLView.as_view("graphql", schema=schema, graphiql=True),
)
